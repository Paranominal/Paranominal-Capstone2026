...
public class AoEAttack : MonoBehaviour
{
    ...
    private void Update()
    {
        //keep the velocity sample fresh on whatever target was last requested.
        //this means by the time PerformAttack(target) is called, we already have
    ...
    /// Use this overload when the caller wants direct control over the AoE position
    /// (trap tiles, predetermined attack points, etc).
    ...
    /// Summary:
    /// Begin the telegraph -> strike sequence aimed at the given target. The actual snapshot
    /// position is computed using the targeting rules (stationary offset, lead, scatter).
    ...
    /// Summary:
    /// Start sampling the target's position so velocity estimates are ready by attack time.
    /// Call this from the behaviour as soon as a target is acquired (optional - PerformAttack
    ...
    //compute the snapshot position using the targeting rules.
    //branches on player speed: toward-caster offset for stationary, lead+scatter for moving.
    ...
    //offset the AoE back toward the caster on the horizontal plane, so the danger zone
    //is visible past the player's body and "back away from caster" is a consistent dodge.
    ...
    //lead the target's velocity slightly, then scatter randomly. the scatter is
    //intentionally larger than typical lead so leading reads as bias, not prediction.
