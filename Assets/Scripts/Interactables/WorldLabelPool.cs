...
public class WorldLabelPool : MonoBehaviour
{
    ...
    // Mark a label as active this frame. Call once per visible label. id should be the anchor's GetInstanceID() so the same object keeps the same entry.
    ...
    // Called after all Show() calls for this frame. Any label that wasn't touched this frame begins fading out.
    public void Flush()
    {
    ...
    // Master on/off gate. Hiding snaps all labels to invisible immediately.
